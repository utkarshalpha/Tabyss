...
/* Substituted values are escaped HERE, at injection time — the template is
 * escaped before fill(), so both halves are safe no matter what a future
 * slide puts in vars (e.g. a user-typed session intention). */
function fill(str, vars) {
  return str.replace(/\{(\w+)\}/g, (_, k) => esc(vars[k] != null ? vars[k] : ""));
}
...
/* Narrative gradient per slide: the deck falls through the abyss into ember
 * (owner design prototype), instead of rotating arbitrary colors. */
const SLIDE_GRADIENTS = {
  intro: ["#6D28D9", "#2E1065"],
  total: ["#7C3AED", "#4C1D95"],
  top_site: ["#A21CAF", "#6D28D9"],
  categories: ["#C026D3", "#86198F"],
  rhythm: ["#DB2777", "#9D174D"],
  rabbit: ["#E11D48", "#881337"],
  focus: ["#EA580C", "#9A3412"],
  sessions: ["#F97316", "#7C2D12"],
};
const PALETTE = [["#6d28d9", "#2e1065"]]; // fallback for unknown slide ids
...
function buildSlides() {
  ...
  // V2's intentional sessions get their own slide when the week had any.
  if (weekFocusSessions.length) {
    const completed = weekFocusSessions.filter((r) => r.outcome === "completed").length;
    const focusedSecs = weekFocusSessions.reduce((sum, r) => sum + r.focusedMs, 0) / 1000;
    const counts = {};
    for (const r of weekFocusSessions) counts[r.intention] = (counts[r.intention] || 0) + 1;
    const top = Object.entries(counts).sort((a, b) => b[1] - a[1])[0];
    out.push({
      id: "sessions",
      vars: {},
      custom: {
        title: "You browsed on purpose 🎯",
        subtitle: `${weekFocusSessions.length} intentional session${weekFocusSessions.length > 1 ? "s" : ""} this week — ${completed} completed, ${fmtShort(focusedSecs)} of focused time.`,
      },
      body: top ? `<span class="schip">Most-set intention: ${esc(top[0])}</span>` : "",
      bodyClass: "statline",
    });
  }
...
function render() {
  ...
  const pal = s.reveal || s.share ? persona.gradient : (SLIDE_GRADIENTS[s.id] || PALETTE[idx % PALETTE.length]);
  setBlob(pal[0], pal[1]);
...
document.addEventListener("keydown", (e) => {
  // Space/Enter on a focused control activates the control, not the deck.
...
(async function init() {
  ...
  try {
    ...
    if (focusData?.ok === true && Array.isArray(focusData.focusSessions)) {
      const startKey = shiftDay(today, -6);
      weekFocusSessions = focusData.focusSessions.filter((r) => r.day >= startKey && r.day <= today);
    }
  } catch (_) { /* Wrapped still works without session history */ }
